...
Сюда же относятся **напоминания по задачам**: `reminder_preset` (1h/1d/1w) и `custom` (ручная дата/время) — их тоже шлёт только `telegram-notify-bot`.
...
### Напоминания по задачам (reminder_preset/custom)

- Миграция: `supabase/migrations/010_task_telegram_reminders.sql` (таблица дедупликации).
- Интервал: `TASK_REMINDER_POLL_MS` (по умолчанию 60_000).
- Логика: отправляется одно сообщение на каждую пару (задача, время напоминания), когда время наступило.
